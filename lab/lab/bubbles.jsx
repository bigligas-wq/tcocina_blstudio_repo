...
// reemplaza colores en una lottie. el map usa la clave "r,g,b" (3 decimales) → [r,g,b,a]
// las lotties wired-outline traen un azul casi-negro [0.071,0.075,0.192] que sobre fondo oscuro
// desaparece; lo recoloreamos en runtime para que se lea sin tocar los acentos brand.
function recolorLottie(data, map){
  const keys = Object.keys(map);
  if(!keys.length) return data;
  const walk = node => {
    if(Array.isArray(node)){ node.forEach(walk); return; }
    if(node && typeof node==="object"){
      if(node.c && node.c.k && Array.isArray(node.c.k) && node.c.k.length>=3 && typeof node.c.k[0]==="number"){
        const k = node.c.k.slice(0,3).map(v=>v.toFixed(3)).join(",");
        if(map[k]) node.c.k = map[k];
      }
...
function Lottie({src, loop=true, autoplay=true, className, style, onDone, recolor}){
  const ref = React.useRef(null);
  React.useEffect(()=>{
    let anim, alive=true;
    fetch(src).then(r=>r.json()).then(data=>{
      if(!alive || !ref.current || !window.lottie) return;
      if(recolor) recolorLottie(data, recolor);
